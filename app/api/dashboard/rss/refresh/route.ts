...
function calculateStats(claims: ProcessedClaim[]): any {
  ...
  const verifiedClaims = claims.filter(claim => claim.isVerified && claim.verdict === 'True').length;
  const debunkedClaims = claims.filter(claim => claim.isVerified && claim.verdict === 'False').length;
  const pendingClaims = claims.filter(claim => !claim.isVerified).length;
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`Refreshing RSS feed for user ${userId}...`);
    ...
    const fetchPromises = RSS_SOURCES.map(source => 
      parseRSSFeed(source.url).catch(error => {
    ...
    console.log(`Successfully refreshed RSS feed with ${claims.length} claims`);
